...
public class APTable
{
    ...
    public void addAPToTable( AccessPoint newAp )
    {
        ...
        System.out.println( "Added new AP to table with id " + newAp.getID()
                + " and BSSID " + newAp.getBSSID() );

        // Check if we want to write out more information about the AP to
        // another debug file.
        if( writeDebugFile )
        {
            WriteFile debugWrite = new WriteFile( tableName + "_debug.txt",
                    true );
     ...
     *            Indicates if you want to automatically include each AP into
     *            the table file if no mapping is found (new AP)
     */
        ...
        for( int i = 0; i < apList.size(); i++ )
        {
            for( int j = 0; j < aps.size(); j++ )
            {
                if( apList.get( i ).getBSSID().equals( aps.get( j ).getBSSID() ) )
                {
                    ...
                    System.out.println( "Value mapped with id "
                            + apList.get( i ).getID() + " and BSSID "
                            + apList.get( i ).getBSSID() + " and RSSI "
                            + apList.get( i ).getRSSI() );
                    break;
                }
            }
            ...
            if( apList.get( i ).getID() < 0 )
            {
                ...
                {
                    // Prompt the user if he wants the program to save the
                    // access point to the table.///
                    // addAPToTable( apList.get( i ) );

                    // Remove this AP from the scan list.
                    // apList
    ...
    public void loadTable( String tableName )
    {
        ...
        try
        {
            ...
            // Create a scanner on the input stream so we can parse the file
            // data.
            ...
            // Loop through each line in the file which contains AP mapping
            // data.
            while( tabScan.hasNext() )
            {
                // Create a new access point that we will be adding to our
                // table.
                ...
                // Read the unique BSSID of the AP that will be used to map to
                // an id value.
        ...
        catch( IOException e )
        {
            ...
            {
                System.out.println( "Unable to read from file " + tableName
                        + ". Do you want to create a file by that name?" );
        ...
        {
            System.out
                    .println( "Bad read - generic unkown error - try checking file format" );
        }
    }
